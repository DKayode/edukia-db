CREATE TABLE IF NOT EXISTS payment_configurations (
  ...
  otp_length INTEGER NOT NULL DEFAULT 6 CHECK (otp_length BETWEEN 4 AND 8),
  otp_ttl_minutes INTEGER NOT NULL DEFAULT 10 CHECK (otp_ttl_minutes >= 1),
  otp_max_attempts INTEGER NOT NULL DEFAULT 3 CHECK (otp_max_attempts >= 1),
  otp_resend_cooldown_seconds INTEGER NOT NULL DEFAULT 60 CHECK (otp_resend_cooldown_seconds >= 0),
  otp_max_resends INTEGER NOT NULL DEFAULT 2 CHECK (otp_max_resends >= 0),
  otp_lock_duration_minutes INTEGER NOT NULL DEFAULT 1440 CHECK (otp_lock_duration_minutes >= 1),
  otp_require_admin_unlock BOOLEAN NOT NULL DEFAULT TRUE,
  otp_auto_unlock_enabled BOOLEAN NOT NULL DEFAULT FALSE,
  otp_block_withdrawal_creation BOOLEAN NOT NULL DEFAULT TRUE,
  otp_provider VARCHAR(30) NOT NULL DEFAULT 'infobip' CHECK (otp_provider IN ('console','infobip')),
